...
const jsPsych = makeJsPsych('resultats_phase3.csv');
...
async function initPhase3() {
  const [stimuliRes, questionsRes] = await Promise.all([
    ...
    fetch('js/questions_phase3.json', { cache: 'no-store' }),
  ]);
  const { phase3: PAIRS } = await stimuliRes.json();
  ...
  const intro = {
    ...
    stimulus: `
      <div class="screen">
        ...
        <p>Dans cette expérience, vous allez écouter des <strong>paires de sons</strong> (Son A puis Son B) et évaluer si le type de chaussure, la surface du sol et l'acoustique ont changé entre les deux.</p>
        <ul>
